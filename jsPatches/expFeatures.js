	...
	(function waitForRemoteConfigResolver() {
		...
		if (
			...
			(!Spicetify.RemoteConfigResolver &&
				!Spicetify.Platform?.RemoteConfigDebugAPI &&
				!Spicetify.Platform?.RemoteConfiguration)
		) {
		...
		const setOverrides = async (overrides) => {
			if (Spicetify.Platform?.RemoteConfigDebugAPI) {
				...
				Spicetify.RemoteConfigResolver.value.setOverrides(
					Spicetify.createInternalMap?.(overrides),
				);
			}
		};
		...
		(async function waitForResolver() {
			if (
				...
				isFallback = true;
				notice.innerText =
					"⚠️ Using fallback mode. Some features may not work.";
			...
			remoteConfiguration =
				Spicetify?.RemoteConfigResolver?.value.remoteConfiguration ??
				(await Spicetify.Platform?.RemoteConfigDebugAPI.getProperties());
		})();
		...
		for (const key of Object.keys(overrideList)) {
			...
			console.warn(
				`[spicetify-exp-features] Removed ${key} from override list`,
			);
			localStorage.setItem(
				"spicetify-exp-features",
				JSON.stringify(overrideList),
...
<input type="text" class="search" placeholder="Search for a feature">
		...
		search.oninput = () => {
			...
			for (const row of rows) {
				...
				row.style.display =
					row.textContent.trim().toLowerCase().includes(query) ||
		...
		for (const name of Object.keys(overrideList)) {
			if (
				!prevSessionOverrideList.includes(name) &&
				remoteConfiguration.values.has(name)
			) {
			...
			if (overrideList[name].values) {
				...
				content.appendChild(
					createSlider(name, feature.description, feature.value),
				);
